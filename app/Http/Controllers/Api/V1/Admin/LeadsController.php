...
class LeadsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [];
        if (!$projectId) {
            return response()->json(['message' => 'Project is required'], 401);
        }
        $lead_view = Lead::all();
        ...
        $data['stages'] = [];
        $count = 0;
        foreach($lead_stages as $k => $lead) {
            $lead_data = Lead::select("leads.id", 'leads.name', 'leads.ref_num', 'leads.sell_do_lead_id as sell_do_id', 'leads.phone', 'leads.sell_do_status as status', 'leads.email', 'leads.created_at as date_time', 'sources.name as source', 'sources.name as subsource','campaigns.campaign_name as campaign', 'projects.name as project')
            ->leftJoin('sources', 'leads.source_id', '=', 'sources.id')
            ->leftJoin('sub_sources', 'leads.subsource_id', '=', 'sub_sources.id')
            ...
            ->whereNotNull('leads.sell_do_fields')
            ->where('leads.sell_do_stage', $lead);
            if ($projectId) {
                $lead_data = $lead_data->where('leads.project_id', $projectId);
            }
            if ($stage) {
                $lead_data = $lead_data->where('leads.sell_do_stage', $stage);
            }
            if ($campaign) {
                $lead_data = $lead_data->where('leads.campaign_id', $campaign);
            }
            if ($source_id) {
                $lead_data = $lead_data->where('leads.source_id', $source_id);
            }
            if ($sub_source_id) {
                $lead_data = $lead_data->where('leads.subsource_id', $sub_source_id);
            }
            $lead_data = $lead_data->get();
            $data['stages'][] = [ "id" => $k, "name" => $lead, "leads" => $lead_data];
    ...
    public function show(Lead $lead, $id)
    {
        if (
        ...
        $lead = Lead::select("id", 'name', 'ref_num as lead_id', 'sell_do_lead_id as sell_do_id', 'phone', 'email')
        ->where('id', $id)
        ->get();

        return response()->json(["code" => 200, "message" => "Success", 'data' => $lead]);
    }
