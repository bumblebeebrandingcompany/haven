        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.leads.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="essential_fields[Full Name]" id="name" value="{{ old('name') }}"
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}"
                        class="form-control" @if (!auth()->user()->is_superadmin) required @endif>
                </div>
                <div class="form-group">
                    <label for="additional_email_key">
                        @lang('messages.additional_email_key')
                    </label>
                    <input type="text" name="essential_fields[Addl Email]" id="additional_email_key"
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="essential_fields[Addl Number]" id="secondary_phone_key"
                        value="{{ old('secondary_phone') }}" class="form-control digit_with_comma">
                    <span class="help-block text-muted">
                        Enter phone number including country code without + sign.
                    </span>
                </div>
                <div class="form-group">
                    <label for="sub_source_id">Select Subsource</label>
                    <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}"
                        name="system_fields[Sub Source]" id="sub_source_id">
                        <option value="" disabled selected>Please Select</option>
                        @foreach ($subSources as $subsource)
                            <option value="{{ $subsource->id }}"
                                {{ (old('system_fields.Sub Source') == $subsource->id) ? 'selected' : '' }}>
                                {{ $subsource->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                @endif
                <input type="hidden" name="sell_do_fields[Sell Do Stage]" id="hidden_stage" value="incoming">
