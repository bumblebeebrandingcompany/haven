...
$(document).on('change', '.toggle_view', function() {
    // Get the selected view (kanban or list)
    let view = $(this).val();
    
    // Construct the base URL with the project ID
    let baseUrl = "{{ route('admin.leads.lead', ['id' => $project->id]) }}";
    ...
    // Store the constructed URL in sessionStorage
    sessionStorage.setItem("leadListUrl", updatedUrl);
