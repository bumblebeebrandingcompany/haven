    ...
    <div class="col-sm-12 d-flex justify-content-between align-items-center">
        ...
        <div class="cta">
            <div class="btn-group btn-group-toggle mr-2">
                <a href="{{ route('admin.leads.lead', ['id' => $project->id, 'view' => 'list']) }}"
                    class="btn btn-outline-secondary @if ($lead_view === 'list') active @endif">List View</a>
                <a href="{{ route('admin.leads.lead', ['id' => $project->id, 'view' => 'kanban']) }}"
                ...
                <div class="row">
                            ...
                            <select class="search form-control" id="project_id">
                                ...
                                @foreach ($projects as $key => $item)
                                    <option value="{{ $item->id }}"
                                        @if (isset($filters['project_id']) && $filters['project_id'] == $item->id) selected @endif>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                        ...
                        <div class="col-md-3 campaigns_div">
                            ...
                            <select class="search form-control" id="campaign_id">
                                ...
                                @foreach ($campaigns as $key => $item)
                                    <option value="{{ $item->id }}"
                                        @if (isset($filters['campaign_id']) && $filters['campaign_id'] == $item->id) selected @endif>{{ $item->campaign_name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select class="search form-control select2" name="source" id="source_id">
                                ...
                                @foreach ($sources as $source)
                                    <option value="{{ $source->id }}"
                                        @if (isset($filters['source']) && $filters['source'] == $item->id) selected @endif>{{ $source->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="search form-control" name="leads_status" id="leads_status">
                            ...
                            <option value="duplicate" @if (isset($filters['leads_status']) && $filters['leads_status'] == 'duplicate') selected @endif>Duplicate
                            </option>
                        </select>
                    </div>
                        ...
                        <div class="col-md-3 mt-auto mb-2">
                            <div class="form-check">
                                <input class="form-check-input search" type="checkbox" id="no_lead_id" value="1"
                                    @if (isset($filters['no_lead_id']) && $filters['no_lead_id'] != 'false') checked @endif>
                    ...
                    @if (auth()->user()->is_superadmin)
                        <div class="col-md-3 additional_columns_to_export_div" style="display: none;">
                        </div>
