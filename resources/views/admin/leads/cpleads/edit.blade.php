    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.cp-leads.update', $cpLead->id) }}" method="post">
                        @csrf
                        @method('PUT') <!-- Add this line for the update method -->

                        <div class="form-group">
                            <label for="name">Name:</label>
                            <input type="text" name="name" class="form-control" value="{{ old('name', $cpLead->name) }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('project_id') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($projects as $id => $entry)
                                    <option value="{{ $id }}" {{ old('project_id', $cpLead->project_id) == $id ? 'selected' : '' }}>
                                        {{ $entry }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="phone" id="phone" class="form-control input_number"
                                value="{{ old('phone', $cpLead->phone) }}" required>
                        </div>

                        <div class="form-group">
                            <label for="email">Email:</label>
                            <input type="text" name="email" class="form-control" value="{{ old('email', $cpLead->email) }}" required>
                        </div>
