...
<form id="BookingFormId" method="POST" enctype="multipart/form-data" action="{{ route('admin.booking.store') }}">
    @includeIf('admin.booking_new.partials.common_details_of_applicant')
    <div class="row">
        <div class="col-md-12">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('overall_sqft') ? 'is-invalid' : '' }}" type="text"
                    name="overall_sqft" id="overall_sqft" value="{{ trim($plotdetail->total_sqfts) }}"
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <div id="NeftDetails" style="display: none;">
                            <div class="form-group">
                                <label for="dd_no">Ref num</label>
                                <input class="form-control {{ $errors->has('neft_details') ? 'is-invalid' : '' }}"
                                    type="text" name="advance_amount[][neft_details]" id="neft_details">

                            </div>
                            <div class="form-group">
                                <label for="dd_no">Select date</label>
                                <input class="form-control {{ $errors->has('neft_date') ? 'is-invalid' : '' }}"
                                    type="date" name="advance_amount[][neft_date]" id="neft_date">
                            </div>
                        </div>
                        <div id="RtgsDetails" style="display: none;">
                            <div class="form-group">
                                <label for="dd_no">Ref num</label>
                                <input class="form-control {{ $errors->has('Rtgs_details') ? 'is-invalid' : '' }}"
                                    type="text" name="advance_amount[][rtgs_details]" id="rtgs_details">
                            </div>
                            <div class="form-group">
                                <label for="dd_no">Select date</label>
                                <input class="form-control {{ $errors->has('neft_date') ? 'is-invalid' : '' }}"
                                    type="date" name="advance_amount[][rtgs_date]" id="rtgs_date">
                            </div>
                        </div>
                        <div id="UpiDetails" style="display: none;">
                            <div class="form-group">
                                <label for="dd_no">Ref num</label>
                                <input class="form-control {{ $errors->has('upi_details') ? 'is-invalid' : '' }}"
                                    type="text" name="advance_amount[][upi_details]" id="upi_details">
                            </div>
                            <div class="form-group">
                                <label for="dd_no">Select date</label>
                                <input class="form-control {{ $errors->has('upi_date') ? 'is-invalid' : '' }}"
                                    type="date" name="advance_amount[][upi_date]" id="upi_date">
                            </div>
                        </div>
                        <div>
...
@section('scripts')
    ...
    <script>
        function calculateTotal() {
            ...
            console.log("Per Sqft:", perSqft);
            console.log("Overall Sqft:", overallSqft);
    ...
    <script>
        ...
        function setupEventListeners() {
            ...
            var NeftDetails = document.querySelectorAll('.payment-container #NeftDetails');
            var RtgsDetails = document.querySelectorAll('.payment-container #RtgsDetails');
            var UpiDetails = document.querySelectorAll('.payment-container #UpiDetails');
            ...
            paymentMode.forEach(function(select, index) {
                ...
                select.onchange = function() {
                    ...
                    NeftDetails[index].style.display = (this.value === '6') ? 'block' : 'none';
                    RtgsDetails[index].style.display = (this.value === '7') ? 'block' : 'none';
                    UpiDetails[index].style.display = (this.value === '8') ? 'block' : 'none';
                ...
                NeftDetails[index].style.display = (select.value === '6') ? 'block' : 'none';
                RtgsDetails[index].style.display = (select.value === '7') ? 'block' : 'none';
                UpiDetails[index].style.display = (select.value === '8') ? 'block' : 'none';
...
<script>
    // Using jQuery to handle the change event of the dropdown
    $(document).ready(function() {
        $('#status').change(function() {
            // If the selected option value is '1' (Booked), show the container, otherwise hide it
            if ($(this).val() == '1') {
                $('#container').show();
            } else {
                $('#container').hide();
